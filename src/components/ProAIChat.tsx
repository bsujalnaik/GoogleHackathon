  ...
  useEffect(() => {
    if (user) {
      const unsub = onSnapshot(doc(db, "users", user.uid), (docSnap) => {
        // setIsPro(docSnap.data()?.isPro === true); // This line is removed
      });
      return () => unsub();
    }
  }, [user]);
      ...
      <div className="flex h-screen w-full bg-gradient-to-br from-background to-zinc-900">
        ...
        <div className="flex-1 flex flex-col h-full w-full min-h-0 px-0 md:px-8 py-2 md:py-6">
          <div className="relative flex flex-col items-center justify-center gap-1 py-2 px-4 border-b border-border/20">
            {/* Animated Halo */}
            {isPro && (
              <span className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 pointer-events-none z-0">
                <span className="block w-16 h-16 rounded-full bg-gradient-to-tr from-yellow-300 via-yellow-400 to-yellow-600 blur-xl opacity-60 animate-spin-slow"></span>
              </span>
            )}
            {/* Bot Icon with Gradient */}
            <div className={`relative z-10 w-14 h-14 rounded-full flex items-center justify-center shadow-lg
              ${isPro ? "bg-gradient-to-br from-yellow-400 to-yellow-600" : "bg-gradient-to-br from-primary to-accent"}
            `}>
              <Bot className="w-8 h-8 text-white drop-shadow-lg" />
              {/* Gold Sparkles */}
              {isPro && (
                <span className="absolute -top-2 -right-2 animate-pulse text-yellow-400">✨</span>
              )}
            </div>
            {/* Pro Badge */}
            {isPro && (
              <span className="mt-2 px-3 py-1 bg-gradient-to-r from-yellow-400 to-yellow-600 text-gray-900 rounded-full text-xs font-bold flex items-center gap-1 shadow border border-yellow-400/60 backdrop-blur">
                <Crown className="w-4 h-4 fill-yellow-500" /> Pro Member
              </span>
            )}
            {/* Header Text */}
            <h3 className={`font-extrabold text-2xl ${isPro ? "bg-gradient-to-r from-yellow-400 to-yellow-600 bg-clip-text text-transparent animate-gradient-x" : "text-foreground"}`}>
              FinSight AI {isPro && "Pro"}
            </h3>
              ...
              <div className="w-full">
                {messages.length === 0 && !isTyping ? (
                    ...
                    messages.map((message, idx) => (
                      ...
                      >
                        {message.type === "ai" && (
                          <div className={`w-7 h-7 rounded-full flex items-center justify-center mr-2 flex-shrink-0 shadow-md ${isPro ? "bg-gradient-to-br from-yellow-400 to-yellow-600" : "bg-gradient-to-br from-primary to-accent"}`}>
                ...
                {isTyping && (
                  <div className="flex gap-2 items-center animate-fade-in mb-1">
                    <div className={`w-7 h-7 rounded-full flex items-center justify-center flex-shrink-0 shadow-md ${isPro ? "bg-gradient-to-br from-yellow-400 to-yellow-600" : "bg-gradient-to-br from-primary to-accent"}`}>
                      <Bot className="w-4 h-4 text-white" />
                    </div>
